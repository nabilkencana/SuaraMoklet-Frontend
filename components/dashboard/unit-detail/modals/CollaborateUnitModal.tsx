...
interface CollaborateUnitModalProps {
  isOpen: boolean;
  complaint: Complaint | null;
  collaborateUnitId: string;
...
export default function CollaborateUnitModal({
  isOpen,
  complaint,
  collaborateUnitId,
  collaborateNote,
    ...
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 animate-fade-in">
      <div className="bg-white rounded-3xl p-6 max-w-lg w-full shadow-2xl border border-slate-100 space-y-5 animate-in fade-in zoom-in-95 duration-150">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-extrabold text-slate-800 text-sm">
                Kolaborasi Unit Lain
              </h3>
              <p className="text-[11px] text-slate-400">
                Ajak unit kerja yang lebih relevan untuk berkolaborasi menangani laporan ini
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={onSubmit} className="space-y-5">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              {collaborateUnitId && (
                <span className="text-[10px] text-red-600 font-bold">
                  Terpilih:{" "}
                  {availableUnits.find((u) => u.id === collaborateUnitId)?.name || collaborateUnitId}
                </span>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              {(availableUnits.length > 0
                ...
                const isSelected = collaborateUnitId === unit.id;
                return (
                  <div
                    key={unit.id}
          ...
          <div className="space-y-2">
            <label className="block text-[10.5px] font-bold text-slate-450 uppercase tracking-wider">
              Alasan Kolaborasi (Pilih Cepat)
            </label>
            <div className="flex flex-wrap gap-1.5">
              {[
                "Dibutuhkan koordinasi lintas unit",
                "Mohon bantuan cek data terkait",
                "Perlu keterlibatan unit terkait",
                "Penanganan bersama",
              ].map((chip) => (
                <button
                  key={chip}
          ...
          <div className="space-y-1.5">
            <label className="block text-[10.5px] font-bold text-slate-450 uppercase tracking-wider">
              Catatan / Instruksi Kolaborasi (Opsional)
            </label>
            <textarea
              rows={3}
              placeholder="Berikan instruksi tambahan atau alasan kolaborasi untuk unit penerima..."
              value={collaborateNote}
